      ...
      <Stack gap="lg">
        ...
        {form.anyCommunityChecked && (
          <>
            ...
            <div className="grid gap-2">
              <Label>
                契約主体 <span className="text-destructive">*</span>
              </Label>
              <RadioGroup
                value={form.contractType}
                onValueChange={form.setContractType}
              >
                <div className="flex items-center gap-6">
            ...
            <div className="grid gap-2">
              <Label>
                会員区分 <span className="text-destructive">*</span>
              </Label>
              <RadioGroup
                value={form.memberCategory}
                onValueChange={form.setMemberCategory}
              >
                <div className="flex items-center gap-6">
            ...
            {form.canAccessAudit && form.auditChecked && (
              <AuditCommunityFields
                ...
                setAuditJoinedAt={(v) => {
                  form.setAuditJoinedAt(v);
                  form.clearFieldError("auditJoinedAt");
                }}
                auditResignedAt={form.auditResignedAt}
                setAuditResignedAt={(v) => {
                  form.setAuditResignedAt(v);
                  form.clearFieldError("auditResignedAt");
                }}
            ...
            {form.canAccessNaikan && form.naikanChecked && (
              <SimpleCommunityFields
                ...
                setJoinedAt={(v) => {
                  form.setNaikanJoinedAt(v);
                  form.clearFieldError("naikanJoinedAt");
                }}
                resignedAt={form.naikanResignedAt}
                setResignedAt={(v) => {
                  form.setNaikanResignedAt(v);
                  form.clearFieldError("naikanResignedAt");
                }}
            ...
            {form.canAccessAi && form.aiChecked && (
              <SimpleCommunityFields
                ...
                setJoinedAt={(v) => {
                  form.setAiJoinedAt(v);
                  form.clearFieldError("aiJoinedAt");
                }}
                resignedAt={form.aiResignedAt}
                setResignedAt={(v) => {
                  form.setAiResignedAt(v);
                  form.clearFieldError("aiResignedAt");
                }}
        ...
        <div className="grid grid-cols-2 items-start gap-6">
          ...
          <FormField
            label="メイ"
            error={form.fieldErrors["firstNameKana"]?.[0]}
          >
            <Input
              placeholder="例: タロウ"
          ...
          {form.subEmails.map((sub, index) => (
            <div key={index} className="flex gap-2">
              <Input
                ...
                value={sub}
                onChange={(e) =>
                  form.handleSubEmailChange(index, e.target.value)
                }
                className="flex-1"
              />
              <Button
        ...
        <FormField label="上場区分">
          <Select
            value={
              form.listingCategoryId
                ? String(form.listingCategoryId)
                : NONE_VALUE
            }
            onValueChange={(v) =>
              form.setListingCategoryId(
          ...
          >
            ...
            <SelectContent className="bg-white">
              ...
              {[
                ...new Set(
                  listingCategories
                    .map((lc) => lc.stockExchangeName)
                    .filter(Boolean),
                ),
              ].map((exchangeName) => {
                const items = listingCategories.filter(
                  (lc) => lc.stockExchangeName === exchangeName,
                );
                return (
        ...
        <FormField label="都道府県">
          <Select
            ...
            onValueChange={(v) =>
              form.setPrefectureId(v === NONE_VALUE ? undefined : Number(v))
            }
          >
            <SelectTrigger className="w-full bg-white">
              <SelectValue placeholder="----" />
        ...
        <div className="grid gap-2">
          <Label>転職意欲</Label>
          <RadioGroup
            value={form.jobChangeIntent}
            onValueChange={form.setJobChangeIntent}
          >
            <div className="flex items-center gap-6">
