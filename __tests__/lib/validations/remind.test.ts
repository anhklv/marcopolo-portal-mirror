import { describe, it, expect } from "vitest";
import { remindSchema, testRemindSchema } from "@/lib/validations/remind";

describe("remindSchema", () => {
  const validData = {
    eventId: 1,
    emailTitle: "【テストイベント】参加可否のご回答をお願いします",
    emailBody: "テスト本文\n{RSVP_URL}",
  };

  it("正常系: 全項目入力", () => {
    const result = remindSchema.safeParse(validData);
    expect(result.success).toBe(true);
  });

  it("異常系: eventId が0", () => {
    const result = remindSchema.safeParse({
      ...validData,
      eventId: 0,
    });
    expect(result.success).toBe(false);
  });

  it("異常系: eventId が負数", () => {
    const result = remindSchema.safeParse({
      ...validData,
      eventId: -1,
    });
    expect(result.success).toBe(false);
  });

  it("異常系: emailTitle が空", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailTitle: "",
    });
    expect(result.success).toBe(false);
  });

  it("異常系: emailTitle が空白のみ → trim後に失敗", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailTitle: "   ",
    });
    expect(result.success).toBe(false);
  });

  it("異常系: emailTitle が200文字超", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailTitle: "あ".repeat(201),
    });
    expect(result.success).toBe(false);
  });

  it("正常系: emailTitle が200文字ちょうど", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailTitle: "あ".repeat(200),
    });
    expect(result.success).toBe(true);
  });

  it("異常系: emailBody が空", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailBody: "",
    });
    expect(result.success).toBe(false);
  });

  it("正常系: emailBody が50000文字ちょうど", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailBody: "あ".repeat(50000),
    });
    expect(result.success).toBe(true);
  });

  it("異常系: emailBody が50000文字超", () => {
    const result = remindSchema.safeParse({
      ...validData,
      emailBody: "あ".repeat(50001),
    });
    expect(result.success).toBe(false);
  });
});

describe("testRemindSchema", () => {
  const validData = {
    eventId: 1,
    emailTitle: "【テストイベント】参加可否のご回答をお願いします",
    emailBody: "テスト本文\n{RSVP_URL}",
  };

  it("正常系: 全項目入力", () => {
    const result = testRemindSchema.safeParse(validData);
    expect(result.success).toBe(true);
  });

  it("異常系: emailTitle が空", () => {
    const result = testRemindSchema.safeParse({
      ...validData,
      emailTitle: "",
    });
    expect(result.success).toBe(false);
  });

  it("異常系: emailBody が空", () => {
    const result = testRemindSchema.safeParse({
      ...validData,
      emailBody: "",
    });
    expect(result.success).toBe(false);
  });

  it("異常系: eventId が欠落", () => {
    const result = testRemindSchema.safeParse({
      emailTitle: "タイトル",
      emailBody: "本文",
    });
    expect(result.success).toBe(false);
  });
});
