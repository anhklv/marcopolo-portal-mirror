...
export default function EventInvitePage({
  ...
  const [memberCategories, setMemberCategories] = useState<("non-member" | "member" | "sponsor" | "observer")[]>([]);
  const [organizations, setOrganizations] = useState<("ベンチャー監査役協会" | "ないかんMeetup")[]>([]);
  ...
  const handleMemberCategoryChange = (category: "non-member" | "member" | "sponsor" | "observer", checked: boolean) => {
    if (checked) {
  ...
  const handleOrganizationChange = (org: "ベンチャー監査役協会" | "ないかんMeetup", checked: boolean) => {
    if (checked) {
                  ...
                  <div className="p-4 space-y-4 max-h-[500px] overflow-y-auto">
                    ...
                    <div className="space-y-2 border-t pt-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="member-non-member-invite"
                            checked={memberCategories.includes("non-member")}
                            onCheckedChange={(checked) =>
                              handleMemberCategoryChange("non-member", checked === true)
                            }
                          />
                          <Label htmlFor="member-non-member-invite" className="cursor-pointer text-sm">非会員</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
